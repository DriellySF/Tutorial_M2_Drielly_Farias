...
function obterColID(nome_col) {
  ...
  var id_col = 0;
  var sql =
    "SELECT ID_COLETOR FROM TblColetor WHERE NOME_COLETOR = " +
    "'" +
    nome_col +
    "'";
  console.log(sql);
...
app.post("/colinsert", urlencodedParser, (req, res) => {
  ...
  res.setHeader("Access-Control-Allow-Origin", "*"); // Isso é importante para evitar o erro de CORS
  sql =
    "INSERT INTO TblColetor (ID_COLETOR, NOME_COLETOR) " +
    "VALUES (" +
    req.body.id +
    ", '" +
    req.body.nome +
...
app.post("/protinsert", urlencodedParser, (req, res) => {
  ...
  res.setHeader("Access-Control-Allow-Origin", "*"); // Isso é importante para evitar o erro de CORS
  sql =
    "INSERT INTO TblPROTOCOLO (ID_PROTOCOLO,ID_COLETOR, NOME_PROTOCOLO) " +
    "VALUES (" +
    req.body.id_prot +
    ", " +
    req.body.id_col +
...
app.post("/coldelete_nome", urlencodedParser, (req, res) => {
  ...
  res.setHeader("Access-Control-Allow-Origin", "*"); // Isso é importante para evitar o erro de CORS
  sql =
    "DELETE FROM TblColetor WHERE NOME_COLETOR = '" + req.body.nome_col + "'";
  var db = new sqlite3.Database(DBPATH); // Abre o banco
  db.run(sql, [], (err) => {
    if (err) {
      throw err;
...
app.listen(port, hostname, () => {
  ...
  console.log(
    `Page server running at http://${hostname}:${port}/protinsertcol`
  );
  console.log(`Page server running at http://${hostname}:${port}/protdelete`);
  console.log(
    `Page server running at http://${hostname}:${port}/protdeletecol`
  );
});
